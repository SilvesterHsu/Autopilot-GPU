...
class CcTarget(Target):
    ...
    def _clone_env(self):
        ...
        self._write_rule("%s['STATIC_AND_SHARED_OBJECTS_ARE_THE_SAME']=True"% env_name)
    ...
    def _check_incorrect_no_warning(self):
        ...
        if illegal_path_list:
            console.warning("//%s: warning='no' should only be used "
                            "for code in third_party." % self.fullname)
        ...
        if self.srcs or self.expanded_deps:
            if not self.data.get('allow_undefined'):
                self._write_rule('%s.Append(LINKFLAGS=["-Xlinker", "--no-undefined"])'
                        % env_name)
            if not self._need_static_library():
                self._write_rule('%s = %s.SharedLibrary("%s", %s, %s)' % (
                        ...
                        self._objs_name(),
                        lib_str))
                self.data['dynamic_cc_library_var'] = self._var_name()
                self._add_target_var('so', self._var_name())
            self._write_rule('%s = %s.SharedLibrary("%s", %s, %s)' % (
                    var_name,
                    env_name,
                    self._target_file_path(),
    ...
    def _need_static_library(self):
        options = self.blade.get_options()
        return config.get_item('cc_library_config', 'generate_static')


    def _cc_library(self):
        if self._need_static_library():
...
class CcBinary(CcTarget):
    ...
    def __init__(self,
        ...
        self.data['dynamic_link'] = not self._need_static_library() or dynamic_link
...
class CcTest(CcBinary):
    ...
    def __init__(self,
        ...
        # Hardcode deps rule to third_party gtest main lib.
